...
It's recommended to determine the secret for authentication at runtime or retrieve it from secure storage solutions such as CyberArk Vault or AWS Secret Manager.

The examples below leverage Windows Integrated Authentication for TLS Datacenter (TPP) and use an API key for TLS Protect Cloud (vAaS). When the API key is provided with the `setup-scheduled-task.ps1` script, it is encrypted using the system's Windows DPAPI key.

TODO: Add examples with service accounts, CyberArk, AWS Secret Manager, Azure, etc.
